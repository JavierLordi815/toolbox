...
%
% Caltech Pedestrian Dataset     Version 3.1.0
...
if(~isempty(vidNm)), filesApi.openVid(vidNm); end
  ...
  function makeLayout()
    ...
    pos = [(ss(3)-1560)/2 (ss(4)-590)/2 1560 590];
    ...
    pObj.hBtDet = uic(pObj.h,btnPr{:},[360 3 100 25],st,'Detect!');
        
    pObj.hCn = [pObj.hObjTp pObj.hBtNew pObj.hBtDel ...
      pObj.hBtPrv pObj.hBtNxt pObj.hCbFix pObj.hBtDet];
    ...
    pSeq.apiOccP = selectorRange(pSeq.h,o4,nStep,[],[.9 .9 .7]);
    pSeq.apiOccH = selectorRange(pSeq.h,o4,nStep,[],[.9 .7 .9]);
    
    pSeq.apiLck = selectorRange(pSeq.h,o4,nStep,[],[.5 1 .5]);
    pSeq.lblRng = uic(pSeq.h,txtPr{:},o4,st,'vs',fs,7,ha,'Center');
    pSeq.lblOccP = uic(pSeq.h,txtPr{:},o4,st,'op',fs,7,ha,'Center');
    pSeq.lblOccH = uic(pSeq.h,txtPr{:},o4,st,'oh',fs,7,ha,'Center');
    ...
    function figResized( h, e ) %#ok<INUSD>
      ...
      set( pLf.hAxIR, ps, [x+650*2*r  y+seqH+32 640*r 480*r] );
      set( pRt.hAxIR, ps, [x+650*3*r  y+seqH+32 640*r 480*r] );
      
      set( pLf.h,   ps, [x        y+seqH+2  640*r 30] );
      set( pRt.h,   ps, [x+650*r  y+seqH+2  640*r 30] );
      set( pSeq.h,  ps, [x+(fW-seqW)/2+2 y-40 seqW seqH+40] );
      % postion pSeq contents
      set(pSeq.hAx,ps,[0 11 seqW seqH]); 
      
      y=1;
      pSeq.apiLck.setPos([0 y seqW 10]);
      set(pSeq.lblLck,ps,[-13 y 12 10]);
      
      y=seqH+11;
      pSeq.apiOccP.setPos([0 y seqW 10]);
      set(pSeq.lblOccP,ps,[-13 y 12 10]); 
      
      y=seqH+21;
      pSeq.apiOccH.setPos([0 y seqW 10]);
      set(pSeq.lblOccH,ps,[-13 y 12 10]); 
  ...
  function api = objMakeApi()
    ...
    apiRng=pSeq.apiRng; apiOccP=pSeq.apiOccP; apiOccH=pSeq.apiOccH; apiLck=pSeq.apiLck;
    ...
    set(pObj.hBtDet, 'callback', @(h,e) objDetect());
    
    apiRng.setRngChnCb(@objSetRng); apiRng.setLockCen(1);
    apiOccP.setRngChnCb(@objSetOccP); apiLck.setRngChnCb(@objSetLck);
    apiOccH.setRngChnCb(@objSetOccH);
    % create api
    ...
    function prepPlay()
      ...
      apiRng.enable(0); apiOccP.enable(0); apiOccH.enable(0); apiLck.enable(0);
    end
    ...
    function seqI = prepSeq()
      ...
      apiRng.enable(isVis); apiOccP.enable(isVis); apiOccH.enable(isVis); apiLck.enable(isVis);
      ...
      apiRng.setRng(rng); apiOccP.setRng(occ); 
      apiLck.setRng(lck); apiOccH.setRng(occ); 
      apiRng.enable([1 n]); apiOccP.enable([s e]); 
      apiLck.enable([s e]); apiOccH.enable([s e]); 
    ...
    function hs = drawRects( flag, ind )
      ...
      switch flag
        case {'panelLf','panelRt','panelIR'}
          ...
          for i=1:n, o=os(i); id=o.id; lbl=A.objLbl(id);
            ...
            hs1=drawRect2(o.pos,o.posv,lims,lbl,o.occl,static,id,ind,-1);
            hs=[hs hs1]; %#ok<AGROW>
          end
        case 'objSeq'
	  ...
	  function hs = drawRect2(pos,posv,lims,lbl,occ,static,id,ind,sid)
        if(colType), t=find(strcmp(lbl,objTypes)); else t=id; end
        col=rectCols(mod( occ,size(rectCols,1))+1,:);
        if(id~=objId), ls='-'; else
          if(ind==objInd), ls='--'; else ls=':'; end; end
        rp = {'lw',2,'color',col,'ls',ls,'rotate',0,'ellipse',0};
        [hs,api]=imRectRot('pos',pos,'lims',lims,rp{:});
        ...
        if( static )
          api.setPosLock(1);
          ht=text(pos(1),pos(2)-10, [ lbl{1} ', ' occTypes{occ+1} ]); 
          hs=[hs ht];
          set(ht,'color','w','FontSize',10,'FontWeight','bold');
        else
          api.setPosSetCb( @(pos) objSetPos(pos(1:4),id,ind,sid) );
	...
	function objSetOccH( rng0, rng1 )
      assert(objId>0); [lf,rt]=apiRng.getBnds(rng0~=rng1); assert(lf>0);
      if(lf>1 && rng0(lf-1)==1), lf=lf-1; rng1(lf)=1; end %extend lf
      if(rt<nStep && rng0(rt+1)==1), rt=rt+1; rng1(rt)=1; end %extend rt
      for i=lf:rt
        ind0=curInd+(i-1)*skip; ind1=objGrpInd(ind0,0);
        occl = 2*ones(ind1-ind0+1,1)*rng1(i);
        A=vbb('setVal',A,objId,'occl',occl,ind0+1,ind1+1);
      end; dispApi.requestUpdate();
	end
  ...
  function api = dispMakeApi()
    % variables
    [srRGB, srLWIR, info, looping, nPlay, replay, needUpdate, dispMode, ...
        timeDisp, hImLf, hImRt, hImIRLf, hImIRRt, hImSeq, hObjCur] = deal([]);   
    ...
    api = struct( 'requestUpdate',@requestUpdate, 'init',@init, 'init2',@init2, ...
        'newRect',@newRect, 'setOffset',@setOffset, ...
        'nFrame',@nFrame, 'nFrameRt', @nFrameRt, 'getImg',@getImg, ...
        'width',@width, 'height',@height, 'setPlay', @setPlay, 'objDetect', @objDetect );
      
    function init( sr1 )
      if(isstruct(srRGB)), srRGB=srRGB.close(); end; delete(hObjCur);
      [srRGB, info, looping, nPlay, replay, needUpdate, dispMode, ...
        timeDisp, hImLf, hImRt, hImIRLf, hImIRRt, hImSeq, hObjCur] = deal([]);      
        
      nPlay=0; replay=0; dispMode=0; looping=1; curInd=0;
      needUpdate=1; srRGB=sr1; skip=1;
      if(isstruct(srRGB)), info=srRGB.getinfo(); else
      ...
      hs = [pLf.hAx pRt.hAx pRt.hAxIR pSeq.hAx];
      ...
      function init2( sr1, sr2 )
          delete(hObjCur);
          [srRGB, srLWIR, info, looping, nPlay, replay, needUpdate, dispMode, ...
              timeDisp, hImLf, hImRt, hImIRLf, hImIRRt, hImSeq, hObjCur] = deal([]);          
          
          nPlay=0; replay=0; dispMode=0; looping=1; curInd=0;
          needUpdate=1; srRGB=sr1; srLWIR=sr2; skip=1;
          ...
          if(isstruct(srLWIR)), info=srLWIR.getinfo(); else
              info=struct('numFrames',0,'width',0,'height',0,'fps',25); end
          if(fps), info.fps=fps; end
          setOffset(nStep-1); setSkip(skip0); setFps(info.fps);
    ...
    function updateDisp()
      ...
      if( dispMode<=0 )
        ...
        % For LWIR image
        set( hFig, 'CurrentAxes', pLf.hAxIR );
        ind=curInd; if(replay), ind=ind-nPlay; end
        hImIRLf = imageFast( hImIRLf, getImgIR(ind) );
        hObjCur=[hObjCur objApi.drawRects('panelIR',ind)];
      end
      % display right panel
      if( dispMode>=0 )
        ...
        % For LWIR image
        set( hFig, 'CurrentAxes', pRt.hAxIR );        
        hImIRRt = imageFast( hImIRRt, getImgIR(ind) );
        hObjCur=[hObjCur objApi.drawRects('panelIR',ind)];
    ...
    function requestUpdate( clearHs )
      ...
      if(clearHs), [hImLf, hImRt, hImIRLf, hImIRRt, hImSeq]=deal([]); end
    ...
    function I=getImg(f)
      srRGB.seek(f); I=srRGB.getframe(); if(ismatrix(I)), I=I(:,:,[1 1 1]); end
    end
    
    function I=getImgIR(f)
      srLWIR.seek(f); I=srLWIR.getframe(); if(ismatrix(I)), I=I(:,:,[1 1 1]); end
    end
  ...
  function api = filesMakeApi()
    % variables
    [fVid1, fVid2, fAnn, tSave, tSave1] = deal([]);
    ...
    function updateMenus()
      m=pMenu; en=enStr{~isempty(fVid1)+1}; nm='VBB Labeler';
      set([m.hVidCls m.hAnnNew m.hAnnOpn],'Enable',en);
      en=enStr{~isempty(fAnn)+1}; set([m.hAnnSav m.hAnnCls],'Enable',en);
      if(~isempty(fVid1)), [~,nm1]=fileparts(fVid1); nm=[nm ' - ' nm1]; end
      set(hFig,'Name',nm); objApi.init(); dispApi.requestUpdate();
    end
    
    function closeVid()      
        fVid1=[]; fVid2 = [];   if(~isempty(fAnn)), closeAnn(); end
        dispApi.init2([], []); updateMenus();
    end
    ...
    function openVid( f )
      ...
      try
        ...
        sr2 = seqIo(fVid2,'r',maxCache);
        dispApi.init2(sr1, sr2); updateMenus();
      catch er
        errordlg(['Failed to load: ' fVid1 '. ' er.message],'Error');
    ...
    function openAnn( f )
      assert(~isempty(fVid1)); if(~isempty(fAnn)), closeAnn(); end
      if( nargin>0 )
        ...
        [f,d]=uigetfile('*.vbb;*.txt','Select Annotation',fVid1(1:end-4));
      end
...
%       f=fileparts( fAnn );
    ...
    function newAnn()
      if(~isempty(fAnn)), closeAnn(); end; fAnn=[fVid1(1:end-3) 'vbb'];
      assert(~isempty(fVid1)); A=vbb('init',dispApi.nFrame());
      updateMenus();
    end
...
api = struct('delete',@delete1, 'enable',@enable, 'setPos',@setPos, ...
  ...
  function draw()
    ...
    for i=1:n, I(:,(1:w)+(i-1)*w,:)=Is(:,:,:,(rng1(i)~=0)+1); end
